...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    nSubsidy = 0 * COIN;
    } else if(nHeight >= 5761 && nHeight <= 7200) {
    nSubsidy = 10 * COIN;
    } else if(nHeight >= 7201 && nHeight <= 10080) {
    nSubsidy = 25 * COIN;
    } else if(nHeight >= 10081 && nHeight <= 11520) {
    nSubsidy = 30 * COIN;
    } else if(nHeight >= 11521 && nHeight <= 17280) {
    nSubsidy = 15 * COIN;
    } else if(nHeight >= 17281 && nHeight <= 20160) {
    nSubsidy = 10 * COIN;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "skynet", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Skynet - Neural Based CryptoCurrency";
        CTransaction txNew;
        txNew.nTime = 1409326750;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1409326750;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 2211379;
        if(fTestNet)
        {
            block.nNonce   = 2211379;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("a7b8e52432ecba524c7d160c9f744caf46570c65235559be44c385c1b43cb2d0"));
        block.print();
