...
{
    uiInterface.ThreadSafeMessageBox(str, _("ColossusCoin"), CClientUIInterface::MSG_ERROR);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("ColossusCoin"), CClientUIInterface::MSG_WARNING);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n";
        ...
        strUsage += "  -conf=<file>           " + _("Specify configuration file (default: ColossusCoin.conf)") + "\n";
        strUsage += "  -pid=<file>            " + _("Specify pid file (default: ColossusCoind.pid)") + "\n";
        ...
        strUsage += "  -port=<port>           " + _("Listen for connections on <port> (default: 7670 or testnet: 8671)") + "\n";
        ...
        strUsage += "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 7671 or testnet: 8671)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ColossusCoin is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  ColossusCoin is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ColossusCoin version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (filesystem::exists(GetDataDir() / "wallet.dat"))
    {
        ...
        if (r == CDBEnv::RECOVER_OK)
        {
            ...
            uiInterface.ThreadSafeMessageBox(msg, _("ColossusCoin"), CClientUIInterface::MSG_WARNING);
        }
