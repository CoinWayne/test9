...
Value stakeforcharity(CWallet *pWallet, const Array &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "stakeforcharity <ColossusCoinaddress> <percent> [Change Address] [min amount] [max amount]\n"
    ...
    if (params.size() > 2) {
        ...
        if (!changeAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ColossusCoin change address");
        else {
            if(!IsMine(*pWallet, changeAddress.Get()))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "ColossusCoin change address not owned");
        }
    }
...
Value sendtoaddress(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 4)
        throw runtime_error(
            "sendtoaddress <ColossusCoinaddress> <amount> [comment] [comment-to]\n"
...
Value sendfrom(CWallet* pWallet, const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom <fromaccount> <toColossusCoinaddress> <amount> [minconf=1] [comment] [comment-to]\n"
            "<amount> is a real and is rounded to the nearest 0.000001"
            + HelpRequiringPassphrase(pWallet));
...
static CScript _createmultisig(CWallet* pWallet, const Array& params)
{
    ...
    for (unsigned int i = 0; i < keys.size(); i++)
    {
        ...
        // Case 1: ColossusCoin address and we have full public key:
